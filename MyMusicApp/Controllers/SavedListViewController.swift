...
extension SavedListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let musicSavedData = self.musicManager.getMusicDatasFromCoreData()[indexPath.row]
        cell.musicSavedData = musicSavedData
        
        // 다 저장되어 있으니 저장을 없앨 것인지 경고창 팝업
        cell.saveButtonPressed = { [weak self] (senderCell) in
            ...
            self.makeRemoveCheckAlert { okAction in
                ...
                if okAction {
                    // 코어데이터에 삭제하도록 전달
                    self.musicManager.deleteMusicFromCoreData(with: musicSavedData) {
        ...
        cell.updateButtonPressed = { [weak self] (senderCell, message) in
            ...
            self.makeMessageAlert(myMessage: message) { updatedMessage, okAction in
                ...
                if okAction {
                    // 새로운 메세지로 변경
                    senderCell.musicSavedData?.myMessage = updatedMessage
                    // 옵셔널 바인딩해서 코어데이터에 업데이트 하도록 전달
                    guard let musicSavedData = senderCell.musicSavedData else { return }
                    self.musicManager.updateMusicToCoreData(with: musicSavedData) {
