...
final class MusicManager {
    ...
    private var musicSavedArrays: [MusicSavedData] = []
    
    func getMusicArraysFromAPI() -> [Music] {
        return musicArrays
    }
    
    func getMusicDatasFromCoreData() -> [MusicSavedData] {
    ...
    // Delete (Music 타입을 가지고 데이터 지우기) ==> 저장되어 있는지 확인하고 지우기
    func deleteMusic(with music: Music, completion: @escaping () -> Void) {
        // 동일한 데이터(제목&가수이름)를 가진 것들을 찾아내서 배열로 리턴
        let musicSavedData = musicSavedArrays.filter { $0.songName == music.songName && $0.artistName == music.artistName
        }
        
        // 전달
        if let targetMusicSaved = musicSavedData.first {
    ...
    // Delete (데이터 지우기) - MusicSavedData타입을 가지고 데이터 지우기
    func deleteMusicFromCoreData(with music: MusicSavedData, completion: @escaping () -> Void) {
    ...
    // Update (수정하기)
    func updateMusicToCoreData(with music: MusicSavedData, completion: @escaping () -> Void) {
    ...
    // Read (데이터 불러오기) (코어데이터에서 과져와서 ==> 현재 모델의 배열에 저장)
    private func  fetchMusicsFromCoreData(completion: () -> Void) {
        musicSavedArrays = coredataManager.getMusicSavedArrayFromCoreData()
        completion()
    }
    
    // 이미 저장된 데이터들인지 확인 (저장 여부 확인 -> 하트 표시 가능)
    func checkWhetherSaved() {
        musicArrays.forEach { music in
            // 코어데이터에 저장된 것들 중 음악 및 가수 이름이 같은 것 찾기
            if musicSavedArrays.contains(where: { $0.songName == music.songName && $0.artistName == music.artistName
            }) {
                // 이미 저장되어있는 데이터라고 설정
                music.isSaved = true
            }
